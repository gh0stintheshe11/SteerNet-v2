...
class NoisyLinear(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            # Noisy forward pass during training
...
class DQN_CNN(nn.Module):
    ...
    def __init__(self, input_channels=4, num_actions=5, input_height=240, input_width=64, sigma_init=0.5):
        ...
        # NoisyNet FC layers (replaces nn.Linear)
...
class HighwayDDQNNoisyAgent:
    ...
    def select_action(self, state, speed, training=True):
        """Action selection - NO EPSILON, NoisyNet handles exploration"""
        with torch.no_grad():
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            speed_tensor = torch.FloatTensor([[speed / 30.0]]).to(self.device)
            
            if training:
                # Training mode: noise is active
    ...
    def update_network(self):
        ...
        # ==================== DOUBLE DQN ====================
        with torch.no_grad():
            # Step 1: ONLINE network selects best actions (no noise for selection)
            self.online_net.eval()
    ...
    def train(self, num_episodes=3000, max_steps_per_episode=1000, print_every=10):
        ...
        print(f"Using: Double DQN + PER + NoisyNet")
        ...
        for episode in range(num_episodes):
            ...
            # Reset noise at start of episode
            self.online_net.reset_noise()
            ...
            if (episode + 1) % print_every == 0:
                ...
                # Calculate crash rate over last 100 episodes
...
def main():
    ...
    agent = HighwayDDQNNoisyAgent(
        ...
        learning_rate=1e-4,
        gamma=0.99,
        batch_size=128,              # Smaller for A30
    ...
    # Save
    agent.plot_training_progress("highway_dqn_grayscale_per_ddqn_NoisyNet.png")
    agent.save_model("highway_dqn_grayscale_per_ddqn_NoisyNet.pth")
    
    # Evaluate
    agent.load_model("highway_dqn_grayscale_per_ddqn_NoisyNet.pth")
