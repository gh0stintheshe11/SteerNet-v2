...
class ReplayBuffer:
    """Experience replay buffer for storing experiences with speed"""
    
    def __init__(self, capacity=10000):
        self.buffer = deque(maxlen=capacity)
    
    def push(self, state, speed, action, reward, next_state, next_speed, done):
        self.buffer.append((state, speed, action, reward, next_state, next_speed, done))
...
class HighwayGrayscaleDQNAgent:
    ...
    def update_network(self):
        ...
        # Sample batch
        states, speeds, actions, rewards, next_states, next_speeds, dones = self.replay_buffer.sample(self.batch_size)
...
def main():
    ...
    # Create agent - ADJUSTED FOR SCHOOL SERVER (RTX 4090 + 251GB RAM)
    agent = HighwayGrayscaleDQNAgent(
        ...
        epsilon_end=0.020,
        epsilon_decay=0.998,
        batch_size=512,              # Increased for RTX 4090 (was 64)
        target_update_freq=2000,
        buffer_capacity=100000,      # Larger buffer for 4 frame stack
        stack_size=stack_size
    )
    #agent.load_model("highway_dqn_grayscale_speed_cnn.pth")
    # Train agent - LONGER TRAINING
    ...
    # Plot progress
    agent.plot_training_progress("training_progress_dqn_grayscale_cnn_with_speed.png")
    
    # Save model
    agent.save_model("highway_dqn_grayscale_cnn_with_speed.pth")
    #agent.save_replay_buffer("replay_buffer.pkl")

    # Evaluate agent
    agent.load_model("highway_dqn_grayscale_cnn_with_speed.pth")
