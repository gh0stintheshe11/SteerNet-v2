...
def main():
    ...
    # Create environment
    stack_size = 16  # 16 frames at 15Hz = 1.07s history for motion prediction (was 4)
    config = {
        "observation": {
    ...
    agent = HighwayGrayscaleDQNAgent(
        ...
        buffer_capacity=100000,      # Reduced for 16 frame stack (~188GB RAM)
        stack_size=stack_size
    )
    #agent.load_model("highway_dqn_grayscale_optimized.pth")
    ...
    # Plot progress
    agent.plot_training_progress("training_progress_dqn_grayscale_optimized.png")
    
    # Save model
    agent.save_model("highway_dqn_grayscale_optimized.pth")
