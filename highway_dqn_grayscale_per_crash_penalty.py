...
class SpeedRewardWrapper(gymnasium.Wrapper):
    ...
    def step(self, action):
        ...
        if hasattr(self.env.unwrapped, 'controlled_vehicles') and len(self.env.unwrapped.controlled_vehicles) > 0:
            ...
            # Add speed to info for NN input
...
class DQN_CNN(nn.Module):
    """CNN-based Deep Q-Network for processing frame stacks with speed input"""
    ...
    def __init__(self, input_channels=4, num_actions=5, input_height=240, input_width=64):
        ...
        # Fully connected layers - add 1 for speed input
        self.fc1 = nn.Linear(conv_output_size + 1, 512)  # +1 for speed
    ...
    def forward(self, x, speed):
        ...
        # speed shape: (batch_size, 1) - normalized speed
        ...
        # Concatenate speed to flattened CNN features
...
class PrioritizedReplayBuffer:
    """Prioritized Experience Replay buffer - samples based on TD error priority"""
    ...
    def save(self, path="replay_buffer.pkl"):
        """Save the prioritized replay buffer to a file"""
        data = {
            'buffer': self.buffer,
        ...
        print(f"PER buffer saved to {path} ({len(self.buffer)} experiences)")
    
    def load(self, path="replay_buffer.pkl"):
        """Load the prioritized replay buffer from a file"""
        ...
        print(f"PER buffer loaded from {path} ({len(self.buffer)} experiences)")
...
class HighwayGrayscaleDQNAgent:
    ...
    def __init__(
        ...
        per_beta=0.4,       # PER: importance sampling start
        per_beta_increment=0.001  # PER: beta annealing
    ):
        self.env = env
    ...
    def select_action(self, state, speed, training=True):
        """Epsilon-greedy action selection with speed input"""
        if training and random.random() < self.epsilon:
            return self.env.action_space.sample()
        
        with torch.no_grad():
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            # Normalize speed to [0, 1] range (max_speed = 30)
    ...
    def update_network(self):
        ...
        # Sample batch (now includes indices and weights for PER)
        ...
        speeds = torch.FloatTensor(speeds).unsqueeze(1).to(self.device)  # Shape: (batch, 1)
        actions = torch.LongTensor(actions).to(self.device)
        rewards = torch.FloatTensor(rewards).to(self.device)
        next_states = torch.FloatTensor(next_states).to(self.device)
        next_speeds = torch.FloatTensor(next_speeds).unsqueeze(1).to(self.device)  # Shape: (batch, 1)
    ...
    def train(self, num_episodes=500, max_steps_per_episode=1000, print_every=10):
        ...
        for episode in range(num_episodes):
            ...
            for step in range(max_steps_per_episode):
                # Select and perform action (pass normalized speed)
                ...
                # Store transition (with normalized speeds)
                self.replay_buffer.push(state, current_speed_normalized, action, reward, 
            ...
            avg_speed = episode_speed_sum / (step + 2)  # +2 because we count initial speed
    ...
    def evaluate(self, num_episodes=10, render=False):
        ...
        for episode in range(num_episodes):
            ...
            while not done and steps < 1000:
                # Select action with speed input
    ...
    def plot_training_progress(self, save_path="training_progress_dqn_grayscale_optimized.png"):
    ...
    def save_model(self, path="highway_dqn_grayscale_optimized.pth"):
    ...
    def load_model(self, path="highway_dqn_grayscale_optimized.pth"):
    ...
    def render_episode(self, num_episodes=5):
        ...
        for episode in range(num_episodes):
            ...
            while not done and steps < 1000:
                ...
                # Update speed
                current_speed = info.get('speed', current_speed)
                current_speed_normalized = current_speed / 30.0
...
def main():
    ...
    # Create environment
    stack_size = 4  # Reverted back to 4 frames
    config = {
        "observation": {
    ...
    env = SpeedRewardWrapper(env, absolute_min_speed=0.0, cutoff_speed=20.0, max_speed=30.0, crash_reward=-50.0)
    ...
    agent = HighwayGrayscaleDQNAgent(
        ...
        buffer_capacity=100000,      # 100k buffer for PER
        stack_size=stack_size,
        per_alpha=0.6,               # PER prioritization
        per_beta=0.4,                # PER importance sampling
        per_beta_increment=0.001     # PER beta annealing
    )
    #agent.load_model("highway_dqn_grayscale_per.pth")
    # Train agent - 3000 episodes
    ...
    # Plot progress
    agent.plot_training_progress("training_progress_dqn_grayscale_per_crash_penalty.png")
    
    # Save model
    agent.save_model("highway_dqn_grayscale_per_crash_penalty.pth")
    #agent.save_replay_buffer("replay_buffer.pkl")

    # Evaluate agent
    agent.load_model("highway_dqn_grayscale_per_crash_penalty.pth")
